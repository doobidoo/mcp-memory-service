...
- **fix(consolidation): give controlled forgetting a durable cursor over the stale tail.** Forgetting only ever saw the memories the normal consolidation horizon had already loaded, and that window never reached older memories, so a corpus large enough to matter was never a candidate for archival. Separating stale selection was not enough on its own: archived records stay eligible, so an unanchored stale query re-selects the same newest-stale page every run and a stable corpus larger than `batch_size` starves forever. Forgetting is now its own phase, runs for the monthly, quarterly and yearly horizons even when the normal horizon returns nothing, and pages through the stale tail from a cursor persisted in `RunTracker`, wrapping to zero once after an empty non-zero page. The backends were made deterministic to match: sqlite-vec and Cloudflare order by `created_at DESC` with a stable id/hash tie-breaker, Milvus sorts its stale rows explicitly instead of trusting iterator order, and Cloudflare now actually applies the `stale_days` argument it had been accepting and ignoring (`last_accessed_at` with a `created_at` fallback, inside D1, before `LIMIT`/`OFFSET`). Archival is now idempotent: archiving deliberately leaves the memory in storage, so every wrap of the cursor used to write another copy of a record that had not changed, and the archive grew without bound on a corpus that did not. A memory that is already in the archive and has not been updated since is skipped instead of re-archived. The stale-tail phase archives and never removes: it runs `ControlledForgettingEngine` in a new archive-only mode, so the engine's delete branch — reached through a `potential_duplicate` heuristic that fires on 80% word overlap between two memories that merely share a page, which templated activity notes satisfy routinely — and its compress-in-place branch, which also drops the original row, are both unreachable from the stale tail. The default behaviour of the engine is unchanged for every other caller. The forgetting cursor is kept in the configured consolidation tracker database where one exists; on a backend without one it falls back to a file beside the archive, then to the service data directory, and is held separately from `run_tracker` so that incremental consolidation's in-flight lock and last-run window are unaffected.
