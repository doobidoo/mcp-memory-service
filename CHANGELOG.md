...
- **fix(web): serve the HTTP app under a reverse-proxy sub-path.** The FastAPI app, the server-rendered dashboard and API-overview pages, the dashboard JavaScript, the SSE test page and the OAuth authorization form all assumed `/` as the deployment root, and an automatically derived OAuth issuer carried no prefix. Behind a proxy mount such as `/memory` every static asset, API call and OAuth redirect resolved outside the mounted application, so the dashboard loaded blank and the OAuth flow advertised an issuer the proxy never served. A new optional `MCP_HTTP_ROOT_PATH` is normalized to canonical URL-unreserved path segments (schemes, queries, fragments, backslashes, percent-encoded or non-unreserved content, traversal and empty segments are rejected; a single trailing slash is stripped), wired into FastAPI's `root_path`, injected as a `<base href>` into rendered documents, and appended to a derived OAuth issuer while an explicitly configured `MCP_OAUTH_ISSUER` still wins. Unset, the value is empty and nothing about single-root deployments changes.
