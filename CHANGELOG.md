...
- **[#706] Hardened `docs/deployment/systemd-service.md` for LAN/network exposure**: Added a "Network exposure hardening" subsection with five concrete recommendations: bind to a specific LAN interface instead of `0.0.0.0`, apply per-source-IP firewall rules (`ufw allow from <LAN_SUBNET>`), restrict parent-directory permissions with `chmod 750` to block world-read of the service root, guidance on TLS termination (Caddy/Nginx reverse proxy) or WireGuard VPN for untrusted networks, and a warning about shared config files (`~/.env`) that may expose secrets to other local users. Also replaced all hardcoded `/home/hkr/` paths in the unit-file templates with the `%h` systemd specifier for portability across deployments. (PR #706)
