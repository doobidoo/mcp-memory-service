...
- **fix(consolidation): report consolidation health from observed state.** Every component check returned a hardcoded string — `'functional'`, `'active'`, `'connected'`, `'verified'`, `'optimal'` — so the health endpoint was green on a system with an unsatisfiable clustering algorithm, an unwritable archive directory, a scheduler that had failed to start, or a storage backend that was unreachable. The checks now inspect the real configuration and the attached engine instances. Clustering resolves whether its configured algorithm is actually satisfiable; forgetting validates its thresholds and probes the archive directory for writability, expanding `~` the way `ControlledForgettingEngine` does and cleaning up after itself; compression requires its engine and a positive `max_summary_length`, and is green when both hold, because there is no remote backend behind it to be unverifiable about; the scheduler check reads the live schedule and requires the exact `consolidation_<horizon>` job set while running, is green while disabled only when a non-empty schedule is explicitly all-disabled, and is unhealthy when unavailable, stopped, unattached or out of sync; storage calls `health_probe()`, where false or an exception is unhealthy and a backend without a probe is degraded rather than assumed healthy. The base probe no longer treats an `error` dictionary or `status="error"` as success, Cloudflare marks an unsuccessful or empty D1 statistics result as an error while preserving a successful all-zero aggregate, Milvus probes with `has_collection`, and Hybrid reflects its primary backend.
