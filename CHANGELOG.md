...
- **fix(hooks): apply protocol-correct port fallback to `session-end.js` `triggerQualityEvaluation`** ([#960](https://github.com/doobidoo/mcp-memory-service/pull/960), fixes [#957](https://github.com/doobidoo/mcp-memory-service/issues/957)): `session-end.js` was not updated alongside the `memory-client.js`/`memory-retrieval.js` fix from PR #952. Applies the same protocol-correct port resolution (omit port for `https://`/`http://` standard ports) to `triggerQualityEvaluation`, restoring hook functionality for Cloudflare Tunnel and reverse proxy deployments.
