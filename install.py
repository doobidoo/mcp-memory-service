...
try:
    from scripts.claude_commands_utils import check_claude_code_cli, install_claude_commands
except ImportError:
...
def detect_gpu():
        ...
        if cuda_path and os.path.exists(cuda_path):
            ...
            try:
                ...
                nvcc_output = subprocess.check_output(
                    [os.path.join(cuda_path, "bin", "nvcc"), "--version"], stderr=subprocess.STDOUT, universal_newlines=True
                )
        ...
        for path in cuda_paths:
            if path and os.path.exists(path):
                ...
                try:
                    # Try to get CUDA version
                    nvcc_output = subprocess.check_output(
                        [os.path.join(path, "bin", "nvcc"), "--version"], stderr=subprocess.STDOUT, universal_newlines=True
                    )
                    for line in nvcc_output.split("\n"):
                        if "release" in line:
                            cuda_version = line.split("release")[-1].strip().split(",")[0].strip()
        ...
        for path in rocm_paths:
            if path and os.path.exists(path):
                ...
                try:
                    ...
                    with open(os.path.join(path, "bin", ".rocmversion")) as f:
                        rocm_version = f.read().strip()
                except (OSError, FileNotFoundError):
                    try:
                        rocm_output = subprocess.check_output(["rocminfo"], stderr=subprocess.STDOUT, universal_newlines=True)
    ...
    if system_info["is_macos"] and system_info["is_arm"]:
        try:
            ...
            result = subprocess.run(["system_profiler", "SPDisplaysDataType"], capture_output=True, text=True, timeout=30)
...
def check_dependencies():
    ...
    try:
        ...
        try:
            ...
            if in_venv:
                print_warning(
                    "pip could not be detected, but you're in a virtual environment. "
    ...
    try:
        ...
        if pip_installed:
            try:
                ...
                if in_venv:
                    print_warning(
                        "Failed to install setuptools with pip. If you're using an alternative package manager "
    ...
    try:
        ...
        if pip_installed:
            try:
                subprocess.check_call([sys.executable, "-m", "pip", "install", "wheel"], stdout=subprocess.DEVNULL)
                ...
                if in_venv:
                    print_warning(
                        "Failed to install wheel with pip. If you're using an alternative package manager "
...
def install_pytorch_macos_intel():
    ...
    try:
        ...
        if python_version >= (3, 13):
            ...
            print_info(
                f"Installing PyTorch {torch_version} for macOS Intel (Python {python_version.major}.{python_version.minor})..."
            )
        ...
        cmd = [sys.executable, "-m", "pip", "install", f"sentence-transformers=={st_version}"]
...
def install_pytorch_macos_arm64():
    ...
    try:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "torch>=2.0.0", "torchvision", "torchaudio"]
        ...
        cmd = [sys.executable, "-m", "pip", "install", "sentence-transformers>=2.2.2"]
...
def install_pytorch_windows(gpu_info):
        ...
        if gpu_info["has_directml"]:
            if major == 2 and minor == 4:
                ...
                try:
                    ...
                    try:
                        subprocess.check_call([sys.executable, "-m", "pip", "install", "torch-directml==0.2.5.dev240914"])
    ...
    try:
        ...
        print_warning(
            "You may need to manually install PyTorch using instructions from https://pytorch.org/get-started/locally/"
        )
        return False
    ...
    try:
        ...
        if not uv_path:
            ...
            if platform.system() == "Windows":
                ...
                possible_paths = [os.path.join(install_dir, "uv"), os.path.expanduser("~/.cargo/bin/uv")]
...
def install_package(args):
    ...
    try:
        ...
        if (
            ...
            and system_info["is_x86"]
            and (sys.version_info >= (3, 13) or using_homebrew_pytorch or args.skip_pytorch)
        ):
    ...
    for config_path in claude_config_paths:
        if config_path.exists():
            ...
            try:
                ...
                env_config = {"MCP_MEMORY_BACKUPS_PATH": str(backups_path), "MCP_MEMORY_STORAGE_BACKEND": storage_backend}
                ...
                if system_info["is_windows"]:
                    ...
                    config["mcpServers"]["memory"] = {"command": "python", "args": [script_path], "env": env_config}
                    ...
                    config["mcpServers"]["memory"] = {
                        ...
                        "args": ["--directory", os.path.abspath("."), "run", "memory"],
                        "env": env_config,
                    }
...
def verify_installation():
    ...
    try:
        ...
        if homebrew_pytorch:
            ...
            print_info(
                "For full functionality including embedding generation, install with: pip install 'mcp-memory-service[ml]'"
            )
        pytorch_installed = False
    ...
    if pytorch_installed or homebrew_pytorch:
        try:
            ...
            if pytorch_installed:
                ...
                st_major, _st_minor = int(st_version[0]), int(st_version[1])
            ...
            try:
                ...
                sentence_transformers.SentenceTransformer("paraphrase-MiniLM-L3-v2")
    ...
    try:
        ...
        print_success("MCP Memory Service is installed correctly")
        return True
    except ImportError:
        print_error("MCP Memory Service is not installed correctly")
        return False
...
def is_legacy_hardware(system_info):
    ...
    if system_info["is_macos"] and system_info["is_x86"]:
        ...
        try:
            ...
            result = subprocess.run(["system_profiler", "SPHardwareDataType"], capture_output=True, text=True, timeout=30)
            if result.returncode == 0:
...
def configure_claude_code_integration(system_info):
    ...
    try:
        ...
        print_success("Created personalized .mcp.json configuration")
        ...
        try:
            result = subprocess.run(["claude", "mcp", "list"], capture_output=True, text=True, timeout=10, cwd=".")
...
def detect_mcp_clients():
    ...
    for path in vscode_settings_paths:
        if path.exists():
            try:
                ...
                if any("mcp" in str(key).lower() or "model-context-protocol" in str(key).lower() for key in settings.keys()):
    ...
    generic_mcp_paths = [Path.home() / ".mcp.json", Path.home() / ".config" / "mcp" / "config.json", Path.cwd() / ".mcp.json"]
    ...
    for client_type, config_path in clients.items():
        try:
            if client_type == "claude_desktop":
                ...
                print_info("  -> Claude Code: Configure via project .mcp.json (see instructions below)")
                success_count += 1
        except Exception as e:
...
def configure_claude_desktop_multi_client(config_path, system_info, storage_backend="sqlite_vec"):
    ...
    try:
        ...
        env_config = {"MCP_MEMORY_STORAGE_BACKEND": storage_backend, "LOG_LEVEL": "INFO"}
        ...
        print_info("  [OK] Claude Desktop: Updated configuration for multi-client access")
        return True
...
def configure_vscode_like_multi_client(config_path, client_type, storage_backend="sqlite_vec"):
    ...
    try:
        ...
        print_info("    Add memory server to your MCP extension with these settings:")
        print_info(f"    - Backend: {storage_backend}")
        if storage_backend == "sqlite_vec":
            print_info("    - Database: shared SQLite-vec database")
        else:
            print_info("    - Database: shared ChromaDB database")
        print_info("    - See generic configuration below for details")
        return True
...
def configure_continue_multi_client(config_path, storage_backend="sqlite_vec"):
    ...
    try:
        ...
        print_info("  [OK] Continue IDE: Updated configuration for multi-client access")
        return True
...
def configure_generic_mcp_multi_client(config_path, storage_backend="sqlite_vec"):
    ...
    try:
        ...
        print_info("  [OK] Generic MCP Client: Updated configuration")
        return True
...
async def test_wal_mode_coordination():
    ...
    try:
        ...
        try:
            ...
            memory2 = Memory(
                content=content2, content_hash=generate_content_hash(content2), tags=["setup", "client2"], memory_type="test"
            )
...
def provide_generic_configuration(storage_backend="sqlite_vec"):
    ...
    if platform.system() == "Windows":
        ...
        print_info("  Alternative: python -m mcp_memory_service.server")
    else:
        print_info(f"  Command: {uv_cmd} --directory {repo_path} run memory")
        print_info("  Alternative: python -m mcp_memory_service.server")
...
def main():
    ...
    parser.add_argument(
        "--storage-backend",
        choices=["chromadb", "sqlite_vec", "auto_detect"],
    ...
    parser.add_argument(
        "--use-homebrew-pytorch", action="store_true", help="Use existing Homebrew PyTorch installation instead of pip version"
    )
    parser.add_argument(
        "--force-pytorch",
        action="store_true",
    ...
    parser.add_argument(
        "--legacy-hardware", action="store_true", help="Optimize installation for legacy hardware (2013-2017 Intel Macs)"
    )
    parser.add_argument(
        "--server-mode", action="store_true", help="Install for server/headless deployment (minimal UI dependencies)"
    )
    parser.add_argument("--enable-http-api", action="store_true", help="Enable HTTP/SSE API functionality")
    parser.add_argument(
        "--migrate-from-chromadb", action="store_true", help="Migrate existing ChromaDB installation to selected backend"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--help-detailed", action="store_true", help="Show detailed hardware-specific installation recommendations"
    )
    parser.add_argument(
        "--generate-docs", action="store_true", help="Generate personalized setup documentation for your hardware"
    )
    parser.add_argument(
        "--setup-multi-client",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-multi-client-prompt", action="store_true", help="Skip the interactive prompt for multi-client setup"
    )
    parser.add_argument(
        "--install-claude-commands", action="store_true", help="Install Claude Code commands for memory operations"
    )
    parser.add_argument(
        "--skip-claude-commands-prompt", action="store_true", help="Skip the interactive prompt for Claude Code commands"
    )
    parser.add_argument(
        "--non-interactive", action="store_true", help="Run in non-interactive mode using default values for all prompts"
    )
    ...
    if args.storage_backend == "sqlite_vec" and not args.skip_pytorch and not args.force_pytorch:
    ...
    if not verify_installation():
        ...
        if system_info["is_macos"] and system_info["is_x86"]:
            ...
            print_info(
                "1. First uninstall current packages: pip uninstall -y torch torchvision torchaudio sentence-transformers"
            )
        ...
        if args.setup_multi_client:
            ...
            try:
                if args.non_interactive:
                    ...
                    print("=" * 60)
                    response = (
                        input("Would you like to configure multi-client access? (y/N, press Enter for N): ").strip().lower()
                    )
                    print("=" * 60 + "\n")
                if response in ["y", "yes"]:
                    print_info("")
    ...
    if system_info["is_macos"] and system_info["is_x86"]:
        ...
        if system_info.get("has_homebrew_pytorch"):
            ...
            if python_version >= (3, 13):
                print_info(
                    "- For optimal performance on Intel Macs with Python 3.13+, torch==2.3.0 and sentence-transformers==3.0.0 are recommended"
                )
                ...
                print_info(
                    "- For optimal performance on Intel Macs, torch==2.0.1 and sentence-transformers==2.2.2 are recommended"
                )
                print_info("- You can manually install these versions with:")
