...
def run_command_safe(cmd, success_msg=None, error_msg=None, silent=False, timeout=None, fallback_in_venv=False):
    """
    ...
    try:
        ...
        if fallback_in_venv:
            ...
            if in_venv:
                fallback_msg = (
                    error_msg
                    or "Command failed, but you're in a virtual environment. If you're using an alternative package manager, this may be normal."
                )
...
def install_package_safe(package, success_msg=None, error_msg=None, fallback_in_venv=True):
    ...
    success, _ = run_command_safe(
        cmd, success_msg=default_success, error_msg=default_error, silent=True, fallback_in_venv=fallback_in_venv
    )
    return success
...
def detect_system():
    ...
    if is_macos:
        try:
            ...
            result = subprocess.run(["brew", "list", "pytorch", "--version"], capture_output=True, text=True)
...
def check_dependencies():
    ...
    success, _ = run_command_safe(
        [sys.executable, "-m", "pip", "--version"], success_msg="pip is installed", silent=True, fallback_in_venv=True
    )
...
def install_pytorch_macos_intel():
    ...
    try:
        ...
        if python_version >= (3, 13):
            ...
            success, _ = run_command_safe(cmd, success_msg="Latest PyTorch installed successfully", silent=False)
...
def choose_storage_backend(system_info, gpu_info, args):
    ...
    print_info("")
    "2" if compatibility["chromadb"]["recommendation"] == "problematic" else "2"

    while True:
        try:
            choice = input("Choose storage backend [1-5] (default: 4 - hybrid): ").strip()
            if not choice:
...
def save_credentials_to_env(credentials):
    ...
    filtered_lines = [line for line in existing_lines if not any(key in line for key in credentials.keys())]
    ...
    print_success("Credentials saved to .env file and current environment")
...
def test_cloudflare_connection(credentials):
    ...
    try:
        ...
        headers = {"Authorization": f"Bearer {credentials['CLOUDFLARE_API_TOKEN']}", "Content-Type": "application/json"}

        # Test API token validity
        response = requests.get("https://api.cloudflare.com/client/v4/user/tokens/verify", headers=headers, timeout=10)
...
def install_storage_backend(backend, system_info):
    ...
    if backend == "cloudflare":
        ...
        return install_package_safe(
            "sqlite-vec", success_msg="SQLite-vec installed successfully", error_msg="Failed to install SQLite-vec"
        )
...
def install_package(args):
    ...
    pip_available, _ = run_command_safe([sys.executable, "-m", "pip", "--version"], silent=True)
...
def install_claude_hooks(args, system_info):
    ...
    try:
        ...
        result = subprocess.run(installer_cmd, cwd=str(claude_hooks_dir), capture_output=True, text=True)
...
def main():
    ...
    parser.add_argument(
        "--storage-backend",
        choices=["cloudflare", "sqlite_vec", "chromadb", "hybrid", "auto_detect"],
    ...
    parser.add_argument(
        "--install-hooks", action="store_true", help="Install Claude Code memory awareness hooks after main installation"
    )
    parser.add_argument(
        "--install-natural-triggers", action="store_true", help="Install Natural Memory Triggers (requires Claude Code)"
    )
    parser.add_argument(
        "--with-ml",
        action="store_true",
    ...
    parser.add_argument(
        "--with-chromadb", action="store_true", help="Include ChromaDB backend support (automatically includes ML dependencies)"
    )
