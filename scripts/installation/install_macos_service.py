...
def install_service(user_level=True):
    ...
    result = subprocess.run(["launchctl", "load", "-w", str(plist_file)], capture_output=True, text=True)
...
def start_service(user_level=True):
    ...
    if not plist_file.exists():
        print("❌ Service is not installed. Run without --start to install first.")
        sys.exit(1)
...
def stop_service(user_level=True):
    ...
    result = subprocess.run(["launchctl", "unload", str(plist_file)], capture_output=True, text=True)
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="macOS LaunchAgent installer for MCP Memory Service")

    # Service level
    parser.add_argument("--user", action="store_true", default=True, help="Install as user LaunchAgent (default)")
    parser.add_argument("--system", action="store_true", help="Install as system LaunchDaemon (requires sudo)")
