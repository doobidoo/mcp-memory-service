    ...
    for content_hash, content, tags_json, created_at in all_memories:
        if detect_corrupted_encoding(content):
            ...
            corrupted_memories.append(
                {"hash": content_hash, "content_preview": content[:200], "tags": tags, "created_at": created_at}
            )
...
def main():
    ...
    parser.add_argument(
        "--db-path", type=str, default="/home/hkr/.local/share/mcp-memory/sqlite_vec.db", help="Path to SQLite database"
    )
    parser.add_argument("--execute", action="store_true", help="Actually delete the corrupted memories (default is dry run)")
