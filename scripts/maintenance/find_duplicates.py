...
def find_duplicates(memories_source, similarity_threshold=0.95):
    ...
    return {"exact": exact_duplicates, "similar": similar_duplicates, "total_memories": len(all_memories)}
...
def analyze_duplicate_group(group):
    ...
    analysis = {"group_size": len(group), "recommended_keep": None, "recommended_delete": [], "reasons": []}
...
def remove_duplicates(db_path, duplicate_groups, dry_run=True):
    ...
    print("\n=== EXACT DUPLICATES ===")
    for _content_hash, group in duplicate_groups["exact"].items():
    ...
    for _similarity_hash, group in duplicate_groups["similar"].items():
...
def main():
    ...
    parser.add_argument("--db-path", type=str, help="Path to SQLite database (if not using API)")
    parser.add_argument("--use-api", action="store_true", help="Use API endpoint from config instead of database")
    parser.add_argument("--execute", action="store_true", help="Actually delete the duplicates (default is dry run)")
    parser.add_argument(
