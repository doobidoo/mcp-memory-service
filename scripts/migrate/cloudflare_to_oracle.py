...
class CloudflareMigrator:
    ...
    def __init__(
        ...
        self.cloudflare_storage: Optional[CloudflareStorage] = None
        self.stats = {
            'total_memories': 0,
            'migrated': 0,
            'skipped': 0,
        ...
        while True:
            ...
            try:
                ...
                if cursor:
                    ...
                    query = f"content_hash > '{cursor}' ORDER BY content_hash LIMIT {self.batch_size}"
                else:
                    query = f"ORDER BY content_hash LIMIT {self.batch_size}"
    ...
    async def _fetch_batch_from_cloudflare(self, cursor: Optional[str], limit: int) -> List[Dict[str, Any]]:
        """
            ...
            for memory in memories:
                try:
                    ...
                    response = await client.post(
                        f"{self.oracle_endpoint}/api/memories",
                        json=payload,
                        headers=headers
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        logger.error(f"Failed to migrate memory {memory['content_hash'][:16]}: {response.status_code} {response.text}")
    ...
    async def verify_migration(self) -> bool:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            response = await client.get(
                f"{self.oracle_endpoint}/api/health/detailed",
                headers=headers
            )
    ...
    async def run(self):
        ...
        try:
            ...
            for i in range(0, len(memories), self.batch_size):
                ...
                logger.info(f"Batch {batch_num} complete: {batch_stats['migrated']} migrated, {batch_stats['skipped']} skipped, {batch_stats['errors']} errors")
...
def main():
    ...
    parser.add_argument(
        '--oracle-endpoint',
        required=True,
        help='Oracle HTTP server endpoint (e.g., http://100.x.x.x:8000)'
    )
    parser.add_argument(
        '--api-key',
        default=os.getenv('MCP_API_KEY'),
        help='Oracle server API key (or set MCP_API_KEY env var)'
    )
    parser.add_argument(
        '--batch-size',
        type=int,
    ...
    migrator = CloudflareMigrator(
        oracle_endpoint=args.oracle_endpoint,
