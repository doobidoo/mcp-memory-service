...
def safe_timestamp_convert(timestamp: str | float | int | None) -> float:
    ...
    try:
        ...
        for i, _doc_id in enumerate(results["ids"]):
            try:
                ...
                clean_metadata = {
                    k: v for k, v in metadata.items() if k not in ["tags", "created_at", "updated_at", "memory_type"]
                }
...
async def check_chroma_data(chroma_path: str) -> int:
    ...
    try:
        chroma_storage = ChromaMemoryStorage(path=chroma_path)
...
async def migrate_memories(
    chroma_path: str, sqlite_path: str, stats: MigrationStats, batch_size: int = 50, skip_duplicates: bool = True
) -> bool:
    ...
    try:
        ...
        sqlite_storage = SqliteVecMemoryStorage(db_path=sqlite_path, embedding_model=EMBEDDING_MODEL_NAME)
        ...
        for i in range(0, stats.total_memories, batch_size):
            ...
            print(
                f"Batch {batch_num}/{total_batches} complete. Progress: {migrated_so_far}/{stats.total_memories} ({percentage:.1f}%)"
            )
...
async def main():
    ...
    if success and stats.failed_migrations == 0:
        ...
        print("\\nNext steps:")
        print("   1. Update your environment: export MCP_MEMORY_STORAGE_BACKEND=sqlite_vec")
