...
class QdrantMigrator:
    ...
    def verify_volume_exists(self) -> bool:
        ...
        result = self.run_command(
            ["docker", "volume", "inspect", self.embedded_volume],
            check=False
        )
    ...
    def start_qdrant_server(self):
        ...
        result = self.run_command(
            ["docker", "ps", "-q", "-f", "name=mcp-memory-qdrant"],
            check=False
        )
        ...
        for i in range(30):
            result = self.run_command(
                ["docker", "ps", "-f", "name=mcp-memory-qdrant", "-f", "health=healthy"],
                check=False
            )
            if result.stdout.strip():
    ...
    def mount_and_read_embedded(self) -> List[Dict[str, Any]]:
        ...
        self.run_command([
            "docker", "run", "-d",
            "--name", self.temp_container,
            "-v", f"{self.embedded_volume}:/qdrant/storage",
            "-p", "6335:6333",  # Use port 6335 to avoid conflicts with server on 6333/6334
        ...
        try:
            ...
            if self.collection_name not in collection_names:
                raise MigrationError(
                    f"Collection '{self.collection_name}' not found in embedded storage. "
