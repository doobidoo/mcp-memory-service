...
class MigrationConfig:
    ...
    def __init__(self):
        self.chroma_path: str | None = None
...
class EnhancedMigrationTool:
    ...
    def __init__(self, config: MigrationConfig):
        self.config = config
        self.stats = {"total": 0, "migrated": 0, "skipped": 0, "failed": 0, "errors": []}
        ...
        try:
            ...
            for i in range(total):
                try:
                    ...
                    # Extract timestamps
                    created_at = self.safe_timestamp_convert(metadata.get("created_at", metadata.get("timestamp")))
                    updated_at = self.safe_timestamp_convert(metadata.get("updated_at", created_at))
    ...
    async def migrate_to_sqlite(self, memories: list[dict[str, Any]]) -> bool:
        ...
        try:
            ...
            self.sqlite_storage = SqliteVecMemoryStorage(
                db_path=self.config.sqlite_path, embedding_model=os.environ.get("MCP_MEMORY_EMBEDDING_MODEL", "all-MiniLM-L6-v2")
            )
                            ...
                            try:
                                ...
                                if self.config.skip_duplicates:
                                    existing = conn.execute(
                                        "SELECT 1 FROM memories WHERE content_hash = ? LIMIT 1", (memory_data["content_hash"],)
                                    ).fetchone()
                                ...
                                self.stats["failed"] += 1
                                self.stats["errors"].append(f"Migration error for {memory_data['content_hash'][:8]}: {str(e)}")
                                if progress_bar:
                                    progress_bar.update(1)
    ...
    async def run(self) -> bool:
        ...
        try:
            ...
            if Path(self.config.sqlite_path).exists() and not self.config.force:
                if not self.config.dry_run:
                    response = input("Target database exists. Overwrite? (y/N): ")
                    if response.lower() != "y":
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Enhanced ChromaDB to SQLite-vec migration tool", formatter_class=argparse.RawDescriptionHelpFormatter
    )

    parser.add_argument("--chroma-path", help="Path to ChromaDB data directory (default: auto-detect)")
    parser.add_argument("--sqlite-path", help="Path for SQLite-vec database (default: same dir as ChromaDB)")
    ...
    parser.add_argument("--no-skip-duplicates", action="store_true", help="Migrate all memories including duplicates")
    parser.add_argument("--backup", help="Path for JSON backup file (default: auto-generate)")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging")
    parser.add_argument("--validate-only", action="store_true", help="Only validate existing SQLite database")
