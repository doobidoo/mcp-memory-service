...
async def embedding_similarity_check(
    source_storage,
    target_storage,
    sample_size: int = 100
    ...
    for content_hash in sample_hashes:
        try:
            ...
            if similarity < 0.99:
                errors.append(
                    f"{content_hash}: Low similarity ({similarity:.4f} < 0.99)"
                )
    ...
    for content_hash in sample_hashes:
        try:
            ...
            if source_tags != target_tags:
                errors.append(
                    f"{content_hash}: Tag mismatch - "
                    f"source: {source_tags}, target: {target_tags}"
                )
...
async def timestamp_preserved(
    ...
    for content_hash in sample_hashes:
        try:
            ...
            created_diff = abs(
                source_memory.created_at - target_memory.created_at
            )
            if created_diff > 0.001:  # 1ms in seconds
                errors.append(
                    f"{content_hash}: created_at mismatch - "
                    f"diff: {created_diff * 1000:.3f}ms"
                )

            # Check updated_at timestamp (within 1ms tolerance)
            updated_diff = abs(
                source_memory.updated_at - target_memory.updated_at
            )
            if updated_diff > 0.001:
                errors.append(
                    f"{content_hash}: updated_at mismatch - "
                    f"diff: {updated_diff * 1000:.3f}ms"
                )
...
async def validate_migration(
    ...
    count_match, count_source, count_target = count_matches(
    ...
    (
        ...
        embedding_avg,
        embedding_errors
    ) = await embedding_similarity_check(
    ...
    metadata_pass, metadata_errors = await metadata_preserved(
        source_storage, target_storage, sample_size
    )
    ...
    timestamp_pass, timestamp_errors = await timestamp_preserved(
        source_storage, target_storage, sample_size
    )
...
Examples:
  ...
  python scripts/migration/validate_migration.py \\
    ...
    parser.add_argument(
        "--source",
        required=True,
        help="Path to source storage (SQLite database or Qdrant directory)"
    )

    parser.add_argument(
        "--target",
        required=True,
        help="Path to target storage (SQLite database or Qdrant directory)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Number of memories to sample for validation (default: 100)"
    )
