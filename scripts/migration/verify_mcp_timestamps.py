...
def check_timestamps():
    ...
    for _i, (mem_id, ts_int, created_float, ts_str, content) in enumerate(samples, 1):
        print(f"\nMemory ID {mem_id}:")
        ...
        if created_float:
            print(
                f"  created_at (float): {created_float} = {datetime.fromtimestamp(created_float).strftime('%Y-%m-%d %H:%M:%S.%f')[:23]}"
            )
        if ts_str:
