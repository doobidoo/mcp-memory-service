...
class MigrationValidator:
    ...
    def __init__(self, sqlite_path: str, chroma_path: str | None = None):
    ...
    async def validate_sqlite_database(self) -> bool:
        ...
        try:
            ...
            required_columns = ["content_hash", "content", "tags", "memory_type", "metadata", "created_at", "updated_at"]
            ...
            for row in cursor:
                ...
                if not content:
                    ...
                    self.report.hash_mismatches.append(f"Memory {memory_id}: hash mismatch (hash: {content_hash[:8]}...)")
                    memory_valid = False
                ...
                if tags:
                    ...
                    if not valid_tags:
                        self.report.tag_issues.append(f"Memory {memory_id}: malformed tags: {tags[:50]}...")

                # Validate timestamps
                if not self.validate_timestamp(created_at, "created_at"):
                    self.report.timestamp_issues.append(f"Memory {memory_id}: invalid created_at: {created_at}")
                    memory_valid = False

                if not self.validate_timestamp(updated_at, "updated_at"):
                    self.report.timestamp_issues.append(f"Memory {memory_id}: invalid updated_at: {updated_at}")

                # Validate metadata
                if metadata:
                    valid_meta, meta_dict = self.validate_metadata(metadata)
                    if not valid_meta:
                        self.report.warnings.append(f"Memory {memory_id}: invalid metadata JSON")

                # Check for encoding issues
                try:
                    content.encode("utf-8")
                except UnicodeEncodeError:
                    self.report.encoding_issues.append(f"Memory {memory_id}: encoding issues in content")
                    memory_valid = False
        ...
        try:
            ...
            if chroma_count > 0:
                ...
                if migration_rate < 95:
                    self.report.add_warning(f"Only {migration_rate:.1f}% of ChromaDB memories were migrated")
                elif migration_rate > 105:
                    self.report.add_warning(f"SQLite has {migration_rate:.1f}% of ChromaDB count (possible duplicates)")
...
def find_databases() -> tuple[str | None, str | None]:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate ChromaDB to SQLite-vec migration", formatter_class=argparse.RawDescriptionHelpFormatter
    )

    parser.add_argument("sqlite_path", nargs="?", help="Path to SQLite-vec database (default: auto-detect)")
    parser.add_argument("--chroma-path", help="Path to original ChromaDB for comparison")
    parser.add_argument("--compare", action="store_true", help="Compare with ChromaDB (requires ChromaDB path)")
    parser.add_argument("--fix", action="store_true", help="Attempt to fix common issues (experimental)")
