...
from .transport import HTTPS_ENABLED, HTTP_HOST, HTTP_PORT, MCP_HTTP_ROOT_PATH
...
def get_oauth_issuer() -> str:
    ...
    if (scheme == "https" and HTTP_PORT != 443) or (scheme == "http" and HTTP_PORT != 80):
        return f"{scheme}://{host}:{HTTP_PORT}{MCP_HTTP_ROOT_PATH}"
    else:
        return f"{scheme}://{host}{MCP_HTTP_ROOT_PATH}"
