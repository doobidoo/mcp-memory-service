...
def detect_mcp_client():
    ...
    try:
        ...
        if parent:
            ...
            # Check for LM Studio
            if (
                "lmstudio" in parent_name
                or "lm-studio" in parent_name
                or "lmstudio" in parent_exe.lower()
            ):
                return "lm_studio"
...
log_level = os.getenv(
    "LOG_LEVEL", "WARNING"
).upper()  # Default to WARNING for performance
...
def check_uv_environment():
    ...
    running_with_uv = "UV_ACTIVE" in os.environ or any(
        "uv" in arg.lower() for arg in sys.argv
    )

    if not running_with_uv:
        logger.info(
            "Memory server is running without UV. For better performance and dependency management, consider using UV:"
        )
...
def configure_environment():
    ...
    if (
        ...
        and system_info.accelerator == AcceleratorType.CUDA
    ):
    ...
    if (
        ...
        and system_info.accelerator == AcceleratorType.ROCm
    ):
...
class MemoryServer:
    def __init__(self):
        ...
        if CONSOLIDATION_ENABLED:
            try:
                ...
                self.consolidation_scheduler = (
                    None  # Will be initialized after consolidator
                )
        ...
        try:
            ...
            logger.info(
                f"Initializing on {platform.system()} {platform.machine()} with Python {platform.python_version()}"
            )
            ...
            logger.info(
                f"Deferring {STORAGE_BACKEND} storage initialization to prevent hanging"
            )
            if MCP_CLIENT == "lm_studio":
                print(
                    f"Deferring {STORAGE_BACKEND} storage initialization to prevent startup hanging",
                    file=sys.stdout,
                    flush=True,
                )
            self.storage = None
        ...
        try:
            ...
            if MCP_CLIENT == "lm_studio":
                print(
                    f"Server capabilities registered successfully!",
                    file=sys.stdout,
                    flush=True,
                )
        except Exception as e:
    ...
    def get_average_query_time(self) -> float:
        ...
        logger.debug(
            f"Average query time: {avg:.2f}ms (from {len(self.query_times)} samples)"
        )
        return round(avg, 2)

    async def send_progress_notification(
        self, operation_id: str, progress: float, message: str = None
    ):
        ...
        try:
            ...
            self.current_progress[operation_id] = {
                "progress": progress,
                "message": message
                or f"Operation {operation_id}: {progress:.0f}% complete",
            ...
            if hasattr(self.server, "send_progress_notification"):
                await self.server.send_progress_notification(
                    progress=progress, progress_token=operation_id, message=message
                )
    ...
    async def _initialize_storage_with_timeout(self):
        ...
        try:
            logger.info(
                f"🚀 EAGER INIT: Starting {STORAGE_BACKEND} storage initialization..."
            )
            logger.info(
                f"🔧 EAGER INIT: Environment check - STORAGE_BACKEND={STORAGE_BACKEND}"
            )
            ...
            if STORAGE_BACKEND == "cloudflare":
                ...
                logger.info(
                    f"   API_TOKEN: {'SET' if CLOUDFLARE_API_TOKEN else 'NOT SET'}"
                )
            ...
            if STORAGE_BACKEND == "sqlite_vec":
                ...
                logger.info(
                    f"🔧 EAGER INIT: SQLite-vec - detected coordination mode: {coordination_mode}"
                )
                ...
                if coordination_mode == "http_client":
                    ...
                    from .utils.http_server_manager import (
                        auto_start_http_server_if_needed,
                    )
                    ...
                    if server_started:
                        ...
                        self.storage = HTTPClientStorage()
                        logger.info(
                            f"✅ EAGER INIT: Started HTTP server and using HTTP client storage"
                        )
                    else:
                        ...
                        logger.info(
                            f"✅ EAGER INIT: HTTP server auto-start failed, using direct SQLite-vec storage"
                        )
                else:
                    ...
                    self.storage = SqliteVecMemoryStorage(SQLITE_VEC_PATH)
                    logger.info(
                        f"✅ EAGER INIT: Using direct SQLite-vec storage at {SQLITE_VEC_PATH}"
                    )
                ...
                logger.info(
                    f"✅ EAGER INIT: CloudflareStorage instance created with index: {CLOUDFLARE_VECTORIZE_INDEX}"
                )
                ...
                if all(
                    ...
                    logger.info(
                        f"🔄 EAGER INIT: Cloudflare config prepared for hybrid storage"
                    )
                else:
                    logger.warning(
                        "🔄 EAGER INIT: Incomplete Cloudflare config, hybrid will run in SQLite-only mode"
                    )
                ...
                logger.info(f"🔍 EAGER INIT: Creating QdrantStorage instance...")
                from .storage.qdrant_storage import QdrantStorage

                self.storage = QdrantStorage(
                    storage_path=MCP_QDRANT_STORAGE_PATH or "/data/qdrant",
                    embedding_model=EMBEDDING_MODEL_NAME or "all-MiniLM-L6-v2",
                )
                logger.info(f"✅ EAGER INIT: QdrantStorage instance created")
            else:
                # Unknown backend - should not reach here due to factory validation
                logger.error(
                    f"❌ EAGER INIT: Unknown storage backend: {STORAGE_BACKEND}"
                )
            ...
            logger.info(
                f"🎉 EAGER INIT: {STORAGE_BACKEND} storage initialization successful"
            )

            # Initialize MemoryService with shared business logic
            self.memory_service = MemoryService(self.storage)
            logger.info(
                f"✅ EAGER INIT: MemoryService initialized with {STORAGE_BACKEND} storage"
            )

            # Verify storage type
            storage_type = self.storage.__class__.__name__
            logger.info(
                f"🔍 EAGER INIT: Final storage type verification: {storage_type}"
            )
    ...
    async def _ensure_storage_initialized(self):
        ...
        if not self._storage_initialized:
            try:
                logger.info(
                    f"🔄 LAZY INIT: Starting {STORAGE_BACKEND} storage initialization..."
                )
                logger.info(
                    f"🔧 LAZY INIT: Environment check - STORAGE_BACKEND={STORAGE_BACKEND}"
                )
                ...
                if STORAGE_BACKEND == "sqlite_vec":
                    ...
                    logger.info(
                        f"🔧 LAZY INIT: SQLite-vec - detected coordination mode: {coordination_mode}"
                    )
                    ...
                    if coordination_mode == "http_client":
                        ...
                        if server_started:
                            ...
                            self.storage = HTTPClientStorage()
                            logger.info(
                                f"✅ LAZY INIT: Started HTTP server and using HTTP client storage"
                            )
                        else:
                            # Fall back to direct SQLite-vec storage
                            import importlib
                            ...
                            SqliteVecMemoryStorage = (
                                storage_module.SqliteVecMemoryStorage
                            )
                            self.storage = SqliteVecMemoryStorage(SQLITE_VEC_PATH)
                            logger.info(
                                f"✅ LAZY INIT: HTTP server auto-start failed, using direct SQLite-vec storage at: {SQLITE_VEC_PATH}"
                            )
                    else:
                        ...
                        self.storage = SqliteVecMemoryStorage(SQLITE_VEC_PATH)
                        logger.info(
                            f"✅ LAZY INIT: Created SQLite-vec storage at: {SQLITE_VEC_PATH}"
                        )
                elif STORAGE_BACKEND == "cloudflare":
                    ...
                    logger.info(
                        f"✅ LAZY INIT: Created Cloudflare storage with Vectorize index: {CLOUDFLARE_VECTORIZE_INDEX}"
                    )
                elif STORAGE_BACKEND == "hybrid":
                    ...
                    if all(
                        ...
                        logger.info(
                            f"🔄 LAZY INIT: Cloudflare config prepared for hybrid storage"
                        )
                    else:
                        logger.warning(
                            "🔄 LAZY INIT: Incomplete Cloudflare config, hybrid will run in SQLite-only mode"
                        )

                    logger.info(
                        f"🔄 LAZY INIT: Creating HybridMemoryStorage instance..."
                    )
                    self.storage = HybridMemoryStorage(
                        sqlite_db_path=SQLITE_VEC_PATH,
                        embedding_model="all-MiniLM-L6-v2",
                    ...
                    logger.info(
                        f"✅ LAZY INIT: Created Hybrid storage at: {SQLITE_VEC_PATH} with Cloudflare sync"
                    )
                else:
                    # Unknown/unsupported backend
                    logger.error("=" * 70)
                    logger.error(
                        f"❌ LAZY INIT: Unsupported storage backend: {STORAGE_BACKEND}"
                    )
                    logger.error("")
                ...
                await self.storage.initialize()
                logger.info(
                    f"✅ LAZY INIT: storage.initialize() completed successfully"
                )

                # Verify the storage is properly initialized
                if (
                    hasattr(self.storage, "is_initialized")
                    and not self.storage.is_initialized()
                ):
                    # Get detailed status for debugging
                    if hasattr(self.storage, "get_initialization_status"):
                        status = self.storage.get_initialization_status()
                        logger.error(
                            f"❌ LAZY INIT: Storage initialization incomplete: {status}"
                        )
                ...
                storage_type = self.storage.__class__.__name__
                logger.info(
                    f"🎉 LAZY INIT: Storage backend ({STORAGE_BACKEND}) initialization successful"
                )
                logger.info(
                    f"🔍 LAZY INIT: Final storage type verification: {storage_type}"
                )

                # Initialize MemoryService with shared business logic
                self.memory_service = MemoryService(self.storage)
                logger.info(
                    f"✅ LAZY INIT: MemoryService initialized with {STORAGE_BACKEND} storage"
                )
                ...
                logger.error(
                    f"❌ LAZY INIT: Failed to initialize {STORAGE_BACKEND} storage: {str(e)}"
                )
    ...
    async def initialize(self):
        ...
        try:
            ...
            if MCP_CLIENT == "lm_studio":
                ...
                print(
                    f"Python: {platform.python_version()}", file=sys.stdout, flush=True
                )

            # Log environment info
            logger.info(
                f"🔧 SERVER INIT: Environment - STORAGE_BACKEND={STORAGE_BACKEND}"
            )
            ...
            logger.info(
                f"⏱️  SERVER INIT: Attempting eager storage initialization (timeout: {timeout_seconds}s)..."
            )
            if MCP_CLIENT == "lm_studio":
                print(
                    f"Attempting eager storage initialization (timeout: {timeout_seconds}s)...",
                    file=sys.stdout,
                    flush=True,
                )
            try:
                ...
                if success:
                    if MCP_CLIENT == "lm_studio":
                        print(
                            "[OK] Eager storage initialization successful",
                            file=sys.stdout,
                            flush=True,
                        )
                    logger.info(
                        "✅ SERVER INIT: Eager storage initialization completed successfully"
                    )
                    ...
                    if hasattr(self, "storage") and self.storage:
                        storage_type = self.storage.__class__.__name__
                        logger.info(
                            f"🔍 SERVER INIT: Eager init resulted in storage type: {storage_type}"
                        )
                else:
                    if MCP_CLIENT == "lm_studio":
                        print(
                            "[WARNING] Eager storage initialization failed, will use lazy loading",
                            file=sys.stdout,
                            flush=True,
                        )
                    logger.warning(
                        "⚠️  SERVER INIT: Eager initialization failed, falling back to lazy loading"
                    )
                ...
                if MCP_CLIENT == "lm_studio":
                    print(
                        "[TIMEOUT] Eager storage initialization timed out, will use lazy loading",
                        file=sys.stdout,
                        flush=True,
                    )
                logger.warning(
                    f"⏱️  SERVER INIT: Storage initialization timed out after {timeout_seconds}s, falling back to lazy loading"
                )
                # Reset state for lazy loading
                self.storage = None
                self._storage_initialized = False
                ...
                if MCP_CLIENT == "lm_studio":
                    print(
                        f"[WARNING] Eager initialization error: {str(e)}, will use lazy loading",
                        file=sys.stdout,
                        flush=True,
                    )
                logger.warning(
                    f"⚠️  SERVER INIT: Eager initialization error: {str(e)}, falling back to lazy loading"
                )
    ...
    async def _initialize_consolidation(self):
        ...
        try:
            if self.consolidator is None:
                ...
                if any(
                    schedule != "disabled"
                    for schedule in CONSOLIDATION_SCHEDULE.values()
                ):
                    self.consolidation_scheduler = ConsolidationScheduler(
                        self.consolidator, CONSOLIDATION_SCHEDULE, enabled=True
                    )
                    ...
                    logger.info(
                        "Consolidation scheduler disabled (all schedules set to 'disabled')"
                    )
    ...
    def register_handlers(self):
        ...
        async def handle_get_prompt(
            ...
            if name == "memory_review":
                ...
                if filter_criteria:
                    if "," in filter_criteria:
                        ...
                        memories = await self.storage.search_by_tag(
                            filter_criteria.split(",")
                        )
                    else:
                        # Assume search query
                        memories = await self.storage.search(
                            filter_criteria, n_results=100
                        )
                else:
                    memories = await self.storage.get_recent_memories(100)

                export_text = (
                    f"Exported {len(memories)} memories in {format_type} format:\n\n"
                )
                ...
                if format_type == "markdown":
                    ...
                    for mem in memories:
                        export_text += (
                            f"[{mem.metadata.created_at_iso}] {mem.content}\n"
                        )
                else:  # json
                    import json
                ...
                similarity_threshold = float(
                    arguments.get("similarity_threshold", "0.95")
                )
                ...
                response_text = (
                    f"Learning session stored successfully!\n\n{learning_note}"
                )
                if not success:
            ...
            return types.GetPromptResult(
                description=f"Result of {name} prompt", messages=messages
            )
                    ...
                    types.Tool(
                        ...
                        inputSchema={
                            ...
                            "properties": {
                                "query": {
                                    "type": "string",
                                    "description": "Query with time expressions or content to recall",
                                },
                                "n_results": {
                                    "type": "number",
                                    "default": 5,
                        ...
                        inputSchema={
                            ...
                            "properties": {
                                ...
                                "start_date": {
                                    "type": "string",
                                    "format": "date",
                                    "description": "Filter memories from this date (YYYY-MM-DD)",
                                },
                                "end_date": {
                                    "type": "string",
                                    "format": "date",
                                    "description": "Filter memories until this date (YYYY-MM-DD)",
                                },
                            },
                            "required": ["tags"],
                        },
                    ...
                    types.Tool(
                        ...
                        inputSchema={
                            ...
                            "properties": {
                                "content_hash": {
                                    "type": "string",
                                    "description": "Memory content hash",
                                }
                            },
                        ...
                        inputSchema={
                            ...
                            "properties": {
                                "before_date": {
                                    "type": "string",
                                    "format": "date",
                                    "description": "Date threshold (YYYY-MM-DD)",
                                },
                                "tag": {
                                    "type": "string",
                                    "description": "Optional tag filter",
                        ...
                        inputSchema={
                            ...
                            "properties": {
                                ...
                                "preserve_timestamps": {
                                    "type": "boolean",
                                    "default": True,
                                    "description": "Preserve original created_at",
                                },
                            },
                            ...
                            inputSchema={
                                ...
                                "properties": {
                                    "time_horizon": {
                                        ...
                                        "description": "Time horizon or omit for all",
                                    }
                                },
                            },
                        ),
                        ...
                        types.Tool(
                            ...
                            inputSchema={
                                ...
                                "properties": {
                                    "content": {
                                        "type": "string",
                                        "description": "Exact content to match",
                                    }
                                },
                                "required": ["content"],
                            },
        ...
        async def handle_call_tool(
            ...
            try:
                ...
                if name == "store_memory":
                    ...
                    if not EXPOSE_DEBUG_TOOLS:
                        return [
                            types.TextContent(
                                type="text",
                                text="Debug tools not enabled. Set MCP_MEMORY_EXPOSE_DEBUG_TOOLS=true to enable.",
                            )
                        ]
                ...
                print(
                    f"ERROR in tool execution: {error_msg}", file=sys.stderr, flush=True
                )
    ...
    async def handle_store_memory(self, arguments: dict) -> List[types.TextContent]:
        ...
        try:
            ...
            if result.get("success"):
                if "memories" in result:
                    ...
                    return [
                        types.TextContent(
                            type="text",
                            text=f"Stored {len(result['memories'])} memory chunks successfully",
                        )
                    ]
                else:
                    # Single memory
                    return [
                        types.TextContent(
                            type="text",
                            text=f"Memory stored successfully: {result['memory']['content_hash']}",
                        )
                    ]
            else:
                return [
            ...
            return [
                types.TextContent(type="text", text=f"Error storing memory: {str(e)}")
            ]
    ...
    async def handle_retrieve_memory(self, arguments: dict) -> List[types.TextContent]:
        ...
        try:
            ...
            if not memories:
                return [
                    types.TextContent(type="text", text="No matching memories found")
                ]
            ...
            for i, memory in enumerate(memories):
                ...
                memory_info.extend(
                    [f"Content: {memory['content']}", f"Hash: {memory['content_hash']}"]
                )
            ...
            return [
                types.TextContent(
                    type="text", text=f"Error retrieving memories: {str(e)}"
                )
            ]
    ...
    async def handle_search_by_tag(self, arguments: dict) -> List[types.TextContent]:
        ...
        try:
            ...
            if result.get("error"):
                return [
                    types.TextContent(
                        type="text", text=f"Error searching by tags: {result['error']}"
                    )
                ]
    ...
    async def handle_delete_memory(self, arguments: dict) -> List[types.TextContent]:
        ...
        try:
            ...
            if result.get("success"):
                return [
                    types.TextContent(
                        type="text", text=f"Memory deleted successfully: {content_hash}"
                    )
                ]
            else:
                return [
            ...
            return [
                types.TextContent(type="text", text=f"Error deleting memory: {str(e)}")
            ]
    ...
    async def handle_delete_by_tag(self, arguments: dict) -> List[types.TextContent]:
        ...
        if not tags:
            return [
                types.TextContent(type="text", text="Error: Tags array is required")
            ]
        ...
        try:
            ...
            return [
                types.TextContent(type="text", text=f"Error deleting by tag: {str(e)}")
            ]
    ...
    async def handle_delete_by_tags(self, arguments: dict) -> List[types.TextContent]:
        ...
        try:
            ...
            await self.send_progress_notification(
                operation_id,
                0,
                f"Starting deletion of memories with tags: {', '.join(tags)}",
            )

            # Execute deletion with progress updates
            await self.send_progress_notification(
                operation_id, 25, "Searching for memories to delete..."
            )
                ...
                count, message = await storage.delete_by_tags_with_progress(
                    ...
                    progress_callback=lambda p, msg: asyncio.create_task(
                        self.send_progress_notification(
                            operation_id, 25 + (p * 0.7), msg
                        )
                    ),
                )
            else:
                await self.send_progress_notification(
                    operation_id, 50, "Deleting memories..."
                )
                count, message = await storage.delete_by_tags(tags)
                await self.send_progress_notification(
                    operation_id, 90, f"Deleted {count} memories"
                )

            # Complete the operation
            await self.send_progress_notification(
                operation_id, 100, f"Deletion completed: {message}"
            )

            return [
                types.TextContent(
                    type="text", text=f"{message} (Operation ID: {operation_id})"
                )
            ]
        except Exception as e:
            logger.error(f"Error deleting by tags: {str(e)}\n{traceback.format_exc()}")
            return [
                types.TextContent(type="text", text=f"Error deleting by tags: {str(e)}")
            ]

    async def handle_delete_by_all_tags(
        self, arguments: dict
    ) -> List[types.TextContent]:
        ...
        try:
            ...
            logger.error(
                f"Error deleting by all tags: {str(e)}\n{traceback.format_exc()}"
            )
            return [
                types.TextContent(
                    type="text", text=f"Error deleting by all tags: {str(e)}"
                )
            ]
    ...
    async def handle_cleanup_duplicates(
        ...
        try:
            ...
            logger.error(
                f"Error cleaning up duplicates: {str(e)}\n{traceback.format_exc()}"
            )
            return [
                types.TextContent(
                    type="text", text=f"Error cleaning up duplicates: {str(e)}"
                )
            ]

    async def handle_update_memory_metadata(
        self, arguments: dict
    ) -> List[types.TextContent]:
        ...
        try:
            ...
            if not content_hash:
                return [
                    types.TextContent(
                        type="text", text="Error: content_hash is required"
                    )
                ]
            ...
            if not updates:
                return [
                    types.TextContent(
                        type="text", text="Error: updates dictionary is required"
                    )
                ]
            ...
            if not isinstance(updates, dict):
                return [
                    types.TextContent(
                        type="text", text="Error: updates must be a dictionary"
                    )
                ]
            ...
            if success:
                ...
                logger.warning(
                    f"Failed to update metadata for memory {content_hash}: {message}"
                )
                return [
                    types.TextContent(
                        type="text", text=f"Failed to update memory metadata: {message}"
                    )
                ]
    ...
    async def handle_consolidate_memories(
        ...
        try:
            ...
            if not time_horizon:
                return [
                    types.TextContent(
                        type="text", text="Error: time_horizon is required"
                    )
                ]
            ...
            if time_horizon not in [
                ...
                return [
                    types.TextContent(
                        type="text",
                        text="Error: Invalid time_horizon. Must be one of: daily, weekly, monthly, quarterly, yearly",
                    )
                ]
...
Memories Archived: {report.memories_archived}"""
    ...
    async def handle_consolidation_status(
        ...
        if not CONSOLIDATION_ENABLED or not self.consolidator:
            return [
                types.TextContent(type="text", text="Consolidation system: DISABLED")
            ]
    ...
    async def handle_consolidation_recommendations(
        ...
        try:
            ...
            if time_horizon not in [
                ...
                return [
                    types.TextContent(type="text", text="Error: Invalid time_horizon")
                ]

            # Get recommendations
            recommendations = await self.consolidator.get_consolidation_recommendations(
                time_horizon
            )
    ...
    async def handle_trigger_consolidation(
        ...
        try:
            ...
            success = await self.consolidation_scheduler.trigger_consolidation(
            ...
            if success:
                ...
                return [
                    types.TextContent(
                        type="text",
                        text=f"Failed to trigger {time_horizon} consolidation",
                    )
                ]
    ...
    async def handle_pause_consolidation(
        ...
        try:
            ...
            # Pause consolidation
            success = await self.consolidation_scheduler.pause_consolidation(
                time_horizon
            )
            ...
            if success:
                ...
                return [
                    types.TextContent(
                        type="text",
                        text=f"Successfully paused {target} consolidation jobs",
                    )
                ]
            else:
                return [
    ...
    async def handle_resume_consolidation(
        ...
        try:
            ...
            success = await self.consolidation_scheduler.resume_consolidation(
                time_horizon
            )
    ...
    async def handle_debug_retrieve(self, arguments: dict) -> List[types.TextContent]:
        ...
        try:
            ...
            results = await debug_retrieve_memory(
                storage, query, n_results, similarity_threshold
            )
            ...
            for i, result in enumerate(results):
                ...
                if result.debug_info:
                    ...
                    if "similarity_threshold" in result.debug_info:
                        memory_info.append(
                            f"Threshold: {result.debug_info['similarity_threshold']:.2f}"
                        )
            ...
            return [
                types.TextContent(
                    type="text", text=f"Error in debug retrieve: {str(e)}"
                )
            ]

    async def handle_exact_match_retrieve(
        self, arguments: dict
    ) -> List[types.TextContent]:
        content = arguments.get("content")
        if not content:
        ...
        try:
            ...
            return [
                types.TextContent(
                    type="text", text=f"Error in exact match retrieve: {str(e)}"
                )
            ]

    async def handle_get_raw_embedding(
        self, arguments: dict
    ) -> List[types.TextContent]:
        content = arguments.get("content")
        if not content:
        ...
        try:
            ...
            return [
                types.TextContent(
                    type="text", text=f"Error getting raw embedding: {str(e)}"
                )
            ]
    ...
    async def handle_recall_memory(self, arguments: dict) -> List[types.TextContent]:
        ...
        try:
            ...
            cleaned_query, (start_timestamp, end_timestamp) = extract_time_expression(
                query
            )
            ...
            if start_timestamp is not None:
                ...
                logger.info(
                    f"Start timestamp: {start_timestamp} ({start_dt.strftime('%Y-%m-%d %H:%M:%S')})"
                )
            if end_timestamp is not None:
                end_dt = datetime.fromtimestamp(end_timestamp)
                logger.info(
                    f"End timestamp: {end_timestamp} ({end_dt.strftime('%Y-%m-%d %H:%M:%S')})"
                )
            ...
            if start_timestamp is None and end_timestamp is None:
                ...
                logger.info(
                    f"Direct parse result: {start_timestamp} to {end_timestamp}"
                )
            ...
            for i, result in enumerate(results):
                ...
                if memory_dt:
                    memory_info.append(
                        f"Timestamp: {memory_dt.strftime('%Y-%m-%d %H:%M:%S')}"
                    )
            ...
            return [
                types.TextContent(
                    type="text", text=f"Error recalling memories: {str(e)}"
                )
            ]

    async def handle_check_database_health(
        self, arguments: dict
    ) -> List[types.TextContent]:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Storage initialization failed during health check: {str(init_error)}"
                )
                return [
                    types.TextContent(
                        type="text",
            ...
            if storage_type == "SqliteVecMemoryStorage":
                ...
                if not hasattr(storage, "conn") or storage.conn is None:
                    ...
                    try:
                        ...
                        if not cursor.fetchone():
                            ...
                            has_model = (
                                hasattr(storage, "embedding_model")
                                and storage.embedding_model is not None
                            )
                            ...
                            db_path = (
                                storage.db_path if hasattr(storage, "db_path") else None
                            )
                ...
                try:
                    if not hasattr(storage, "primary") or storage.primary is None:
                        ...
                        if (
                            not hasattr(primary_storage, "conn")
                            or primary_storage.conn is None
                        ):
                            is_valid = False
                            message = (
                                "Hybrid storage: SQLite connection is not initialized"
                            )
                            stats = {
                                "status": "error",
                            ...
                            if not cursor.fetchone():
                                ...
                                if hasattr(storage, "secondary") and storage.secondary:
                                    sync_service = getattr(
                                        storage, "sync_service", None
                                    )
                                    if sync_service and getattr(
                                ...
                                stats = {
                                    ...
                                    "database_path": getattr(
                                        primary_storage, "db_path", SQLITE_VEC_PATH
                                    ),
                    ...
                    stats = {"status": "error", "error": str(e), "backend": "hybrid"}
            ...
            performance_stats = {}
            if hasattr(storage, "get_performance_stats") and callable(
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not get performance stats: {str(perf_error)}"
                    )
            ...
            if hasattr(storage, "get_initialization_status") and callable(
                storage.get_initialization_status
            ):
                try:
                    server_stats["storage_initialization"] = (
                        storage.get_initialization_status()
                    )
                except Exception:
                    pass
            ...
            result = {
                ...
                "statistics": stats,
                "performance": {"storage": performance_stats, "server": server_stats},
            }
            ...
            return [
                types.TextContent(
                    type="text", text=f"Error checking database health: {str(e)}"
                )
            ]

    async def handle_recall_by_timeframe(
        self, arguments: dict
    ) -> List[types.TextContent]:
        ...
        try:
            ...
            end_date = datetime.fromisoformat(
                arguments.get("end_date", arguments["start_date"])
            ).date()
            n_results = min(arguments.get("n_results", 5), 100)  # Enforce max 100
            ...
            logger.info(
                f"Start timestamp: {start_timestamp} ({datetime.fromtimestamp(start_timestamp).strftime('%Y-%m-%d %H:%M:%S')})"
            )
            logger.info(
                f"End timestamp: {end_timestamp} ({datetime.fromtimestamp(end_timestamp).strftime('%Y-%m-%d %H:%M:%S')})"
            )
            ...
            if not results:
                return [
                    types.TextContent(
                        type="text",
                        text=f"No memories found from {start_date} to {end_date}",
                    )
                ]
            ...
            for i, result in enumerate(results):
                ...
                if memory_timestamp:
                    memory_info.append(
                        f"Timestamp: {memory_timestamp.strftime('%Y-%m-%d %H:%M:%S')}"
                    )
            ...
            logger.error(
                f"Error in recall_by_timeframe: {str(e)}\n{traceback.format_exc()}"
            )
            return [
                types.TextContent(
                    type="text", text=f"Error recalling memories: {str(e)}"
    ...
    async def handle_delete_by_timeframe(
        self, arguments: dict
    ) -> List[types.TextContent]:
        ...
        try:
            ...
            count, message = await storage.delete_by_timeframe(
                start_date, end_date, tag
            )
            return [
                types.TextContent(
                    type="text", text=f"Deleted {count} memories: {message}"
                )
            ]
            ...
            return [
                types.TextContent(
                    type="text", text=f"Error deleting memories: {str(e)}"
                )
            ]

    async def handle_delete_before_date(
        self, arguments: dict
    ) -> List[types.TextContent]:
    ...
    async def handle_ingest_document(self, arguments: dict) -> List[types.TextContent]:
        ...
        try:
            ...
            if loader is None:
                ...
                supported_exts = ", ".join(
                    f".{ext}" for ext in SUPPORTED_FORMATS.keys()
                )
                return [
                    types.TextContent(
                        type="text",
            ...
            success_rate = (
                (chunks_stored / chunks_processed * 100) if chunks_processed > 0 else 0
            )
            ...
            logger.info(
                f"Document ingestion completed: {chunks_stored}/{chunks_processed} chunks stored"
            )
            return [types.TextContent(type="text", text="\n".join(result_lines))]
            ...
            return [
                types.TextContent(
                    type="text", text=f"Error ingesting document: {str(e)}"
                )
            ]
    ...
    async def handle_ingest_directory(self, arguments: dict) -> List[types.TextContent]:
        ...
        try:
            ...
            for file_path in files_to_process:
                try:
                    logger.info(
                        f"Processing file {files_processed + 1}/{len(files_to_process)}: {file_path.name}"
                    )
                    ...
                    async for chunk in loader.extract_chunks(file_path):
                        ...
                        try:
                            ...
                            if success:
                                ...
                                all_errors.append(
                                    f"{file_path.name} chunk {chunk.chunk_index}: {error}"
                                )

                        except Exception as e:
                            all_errors.append(
                                f"{file_path.name} chunk {chunk.chunk_index}: {str(e)}"
                            )
            ...
            processing_time = time.time() - start_time
            success_rate = (
                (total_chunks_stored / total_chunks_processed * 100)
            ...
            if all_errors:
                ...
                if len(all_errors) > error_limit:
                    result_lines.append(
                        f"   ... and {len(all_errors) - error_limit} more errors"
                    )

            logger.info(
                f"Directory ingestion completed: {total_chunks_stored}/{total_chunks_processed} chunks from {files_processed} files"
            )
            return [types.TextContent(type="text", text="\n".join(result_lines))]
            ...
            return [
                types.TextContent(
                    type="text", text=f"Error ingesting directory: {str(e)}"
                )
            ]
...
async def async_main():
    ...
    if MCP_CLIENT == "lm_studio":
        print(
            "\n=== MCP Memory Service System Diagnostics ===",
            file=sys.stdout,
            flush=True,
        )
        print(
            f"OS: {system_info.os_name} {system_info.architecture}",
            file=sys.stdout,
            flush=True,
        )
        print(f"Python: {platform.python_version()}", file=sys.stdout, flush=True)
        print(
            f"Hardware Acceleration: {system_info.accelerator}",
            file=sys.stdout,
            flush=True,
        )
        print(f"Memory: {system_info.memory_gb:.2f} GB", file=sys.stdout, flush=True)
        print(
            f"Optimal Model: {system_info.get_optimal_model()}",
            file=sys.stdout,
            flush=True,
        )
        print(
            f"Optimal Batch Size: {system_info.get_optimal_batch_size()}",
            file=sys.stdout,
            flush=True,
        )
    ...
    try:
        ...
        while retry_count <= max_retries and not init_success:
            if retry_count > 0:
                logger.warning(
                    f"Retrying initialization (attempt {retry_count}/{max_retries})..."
                )
            ...
            try:
                ...
                logger.warning(
                    "Async initialization timed out. Continuing with server startup."
                )
        ...
        running_in_docker = os.path.exists("/.dockerenv") or os.environ.get(
            "DOCKER_CONTAINER", False
        )

        if standalone_mode:
            logger.info(
                "Running in standalone mode - keeping server alive without active client"
            )
            if MCP_CLIENT == "lm_studio":
                print(
                    "MCP Memory Service running in standalone mode",
                    file=sys.stdout,
                    flush=True,
                )
            ...
            async with mcp.server.stdio.stdio_server() as (read_stream, write_stream):
                ...
                if running_in_docker:
                    logger.info(
                        "Detected Docker environment - ensuring proper stdio handling"
                    )
                    if MCP_CLIENT == "lm_studio":
                        print(
                            "MCP Memory Service running in Docker container",
                            file=sys.stdout,
                            flush=True,
                        )
                ...
                try:
                    ...
                    if type(e).__name__ == "ExceptionGroup" or "ExceptionGroup" in str(
                        type(e)
                    ):
                        error_str = str(e)
                        ...
                        if (
                            ...
                            logger.info(
                                "LM Studio sent a cancelled notification - this is expected behavior"
                            )
...
def main():
    ...
    try:
        ...
        if os.path.exists("/.dockerenv") or os.environ.get("DOCKER_CONTAINER", False):
            ...
            if MCP_CLIENT == "lm_studio":
                print(
                    "MCP Memory Service starting in Docker mode",
                    file=sys.stdout,
                    flush=True,
                )
