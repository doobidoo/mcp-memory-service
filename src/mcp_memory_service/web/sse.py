...
class SSEManager:
        ...
        if connection_id in self.connections:
            ...
            try:
                close_event = SSEEvent(
                    event_type="connection_closed", data={"connection_id": connection_id, "duration_seconds": duration}
                )
    ...
    async def broadcast_event(self, event: SSEEvent, connection_filter: set[str] | None = None):
        """Broadcast an event to all or filtered connections."""
        if not self.connections:
            return

        target_connections = (
            connection_filter.intersection(self.connections.keys()) if connection_filter else self.connections.keys()
        )
            ...
            while True:
                try:
                    ...
                    event_data = {
                        ...
                        "data": json.dumps({"timestamp": event.timestamp, **event.data}),
                    }
...
def create_health_update_event(status: str, details: dict[str, Any] = None) -> SSEEvent:
    ...
    return SSEEvent(
        event_type="health_update", data={"status": status, "details": details or {}, "message": f"System status: {status}"}
    )
