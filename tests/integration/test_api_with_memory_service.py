...
def mock_storage():
    ...
    storage = AsyncMock()
    # Configure required property/method return values
    storage.max_content_length = None  # No content length limit
    storage.get_content_limits.return_value = {"max_content_length": None}
    storage.get_stats.return_value = {"total_memories": 0, "total_tags": 0}
    return storage
...
@pytest.mark.skip(reason="Mock storage needs comprehensive update for hybrid search - use real integration tests instead")
@pytest.mark.asyncio
async def test_api_store_memory_uses_service(mock_storage):
    ...
    result = await service.store_memory(content="Test API storage", tags=["api", "test"], memory_type="note")
...
async def test_api_store_memory_hostname_from_header(mock_storage):
    ...
    await service.store_memory(content="Test with hostname", tags=["test"], client_hostname="client-machine")
...
async def test_api_store_memory_hostname_from_request_body(mock_storage):
    ...
    await service.store_memory(content="Test", client_hostname="body-hostname")
...
async def test_api_list_memories_tag_filter(mock_storage):
    ...
    await service.list_memories(page=1, page_size=10, tag="important")
...
async def test_api_list_memories_type_filter(mock_storage):
    ...
    await service.list_memories(page=1, page_size=10, memory_type="reference")
...
async def test_api_list_memories_combined_filters(mock_storage):
    ...
    await service.list_memories(page=1, page_size=10, tag="work", memory_type="task")
...
async def test_api_semantic_search_uses_service(mock_storage, sample_memory):
    ...
    # Mock returns MemoryQueryResult list (with relevance scores)
    mock_result = MemoryQueryResult(memory=sample_memory, relevance_score=0.95)
    mock_storage.retrieve.return_value = [mock_result]
    mock_storage.get_all_tags.return_value = []  # For hybrid search tag cache

    service = MemoryService(storage=mock_storage)

    result = await service.retrieve_memories(query="test query", page_size=5)
...
async def test_end_to_end_workflow_with_real_storage(temp_db):
    ...
    try:
        ...
        store_result = await service.store_memory(
            content="End-to-end test memory", tags=["e2e", "integration"], memory_type="test"
        )
...
async def test_http_api_store_memory_endpoint(temp_db):
    ...
    try:
        ...
        response = client.post(
            "/api/memories", json={"content": "HTTP API test memory", "tags": ["http", "api", "test"], "memory_type": "note"}
        )
...
async def test_http_api_list_memories_endpoint(temp_db):
    ...
    try:
        ...
        service = MemoryService(storage=storage)
        for i in range(5):
            await service.store_memory(content=f"Test memory {i}", tags=["test"], memory_type="note")
...
async def test_http_api_search_endpoint(temp_db):
    ...
    try:
        ...
        await service.store_memory(
            content="Python programming language tutorial", tags=["python", "tutorial"], memory_type="reference"
        )

        # Make HTTP POST request for search
        client = TestClient(app)
        response = client.post("/api/search", json={"query": "python tutorial", "limit": 5})
...
async def test_http_api_search_by_tag_endpoint(temp_db):
    ...
    try:
        ...
        service = MemoryService(storage=storage)
        await service.store_memory(content="Important work item", tags=["important", "work"], memory_type="task")
        await service.store_memory(content="Personal note", tags=["personal"], memory_type="note")

        # Search by tag via HTTP
        client = TestClient(app)
        response = client.post("/api/search/by-tag", json={"tags": ["important"], "limit": 10})
...
async def test_http_api_get_memory_by_hash_endpoint(temp_db):
    ...
    try:
        ...
        # Store a memory
        service = MemoryService(storage=storage)
        store_result = await service.store_memory(content="Memory to retrieve", tags=["test"], memory_type="note")
        content_hash = store_result["memory"]["content_hash"]
...
async def test_http_api_delete_memory_endpoint(temp_db):
    ...
    try:
        ...
        # Store a memory
        service = MemoryService(storage=storage)
        store_result = await service.store_memory(content="Memory to delete", tags=["test"], memory_type="note")
        content_hash = store_result["memory"]["content_hash"]
...
async def test_http_api_pagination_with_real_data(temp_db):
    ...
    try:
        ...
        service = MemoryService(storage=storage)
        for i in range(25):
            await service.store_memory(content=f"Pagination test {i}", tags=["pagination"], memory_type="note")
...
async def test_http_api_client_hostname_header(temp_db):
    ...
    try:
        ...
        response = client.post(
            "/api/memories",
            json={"content": "Test with hostname", "tags": ["test"]},
...
async def test_http_api_complete_crud_workflow(temp_db):
    ...
    try:
        ...
        create_response = client.post(
            "/api/memories", json={"content": "CRUD test memory", "tags": ["crud", "test"], "memory_type": "note"}
        )
        ...
        search_response = client.post("/api/search", json={"query": "CRUD test", "limit": 5})
