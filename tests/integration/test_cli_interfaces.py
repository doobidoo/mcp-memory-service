...
class TestCLIInterfaces:
    ...
    def test_memory_command_backward_compatibility(self):
        ...
        result = subprocess.run(
            ["uv", "run", "memory", "--help"], capture_output=True, text=True, timeout=10, cwd=current_dir.parent.parent
        )
    ...
    def test_memory_command_exists(self):
        ...
        result = subprocess.run(["uv", "run", "memory", "--help"], capture_output=True, text=True, cwd=current_dir.parent.parent)
        assert result.returncode == 0
        assert "MCP Memory Service" in result.stdout
    ...
    def test_memory_server_command_exists(self):
        ...
        result = subprocess.run(
            ["uv", "run", "memory-server", "--help"], capture_output=True, text=True, cwd=current_dir.parent.parent
        )
        assert result.returncode == 0
        assert "MCP Memory Service" in result.stdout
    ...
    def test_mcp_memory_server_command_exists(self):
        ...
        result = subprocess.run(
            ["uv", "run", "mcp-memory-server", "--help"], capture_output=True, text=True, cwd=current_dir.parent.parent
        )
    ...
    def test_memory_server_version_flag(self):
        ...
        result = subprocess.run(
            ["uv", "run", "memory-server", "--version"], capture_output=True, text=True, cwd=current_dir.parent.parent
        )
        assert result.returncode == 0
        assert "MCP Memory Service" in result.stdout
        assert "deprecated" in result.stderr.lower()
    ...
    def test_memory_server_vs_memory_server_subcommand(self):
        ...
        # Both should mention debug option (chroma-path removed in v9.0)
    ...
    def test_no_cli_conflicts_during_import(self):
        ...
        try:
            ...
            from mcp_memory_service.cli.main import main, memory_server_main, server
...
class TestCLIFunctionality:
    ...
    def test_memory_status_command(self):
        ...
        result = subprocess.run(
            ["uv", "run", "memory", "status"], capture_output=True, text=True, timeout=30, cwd=current_dir.parent.parent
        )
        ...
        if result.returncode == 0:
            ...
            assert "Version:" in result.stdout  # Don't hardcode version
        else:
...
class TestCLIRobustness:
    ...
    def test_environment_variable_passing(self):
        ...
        # Test memory server command with storage-backend option
        env = os.environ.copy()
        result = subprocess.run(
            [
                "uv",
                ...
                """
import os
print(f"MCP_MEMORY_STORAGE_BACKEND={os.environ.get('MCP_MEMORY_STORAGE_BACKEND', 'NOT_SET')}")
# Test that CLI can be imported without errors
from mcp_memory_service.cli.main import cli, server
...
class TestCLIPerformance:
    ...
    def test_cli_startup_time(self):
        ...
        result = subprocess.run(
            ["uv", "run", "memory", "--help"], capture_output=True, text=True, timeout=30, cwd=current_dir.parent.parent
        )
    ...
    def test_memory_version_performance(self):
        ...
        commands = [["uv", "run", "memory", "--version"], ["uv", "run", "memory-server", "--version"]]

        for cmd in commands:
            start_time = time.time()
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=15, cwd=current_dir.parent.parent)
            elapsed = time.time() - start_time

            assert result.returncode == 0
            assert "MCP Memory Service" in result.stdout  # Don't hardcode version
