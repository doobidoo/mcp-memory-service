...
class TestModelMigration:
    ...
    async def test_content_preservation(self, temp_storage_path, monkeypatch):
        ...
        for memory in after_memories:
            ...
            assert memory.content == before_data["content"], (
                f"Content changed for {memory.content_hash[:8]}"
            )

            # Tags must match (set equality)
            assert set(memory.tags) == before_data["tags"], (
                f"Tags changed for {memory.content_hash[:8]}"
            )

            # Metadata must match (dict equality)
            assert memory.metadata == before_data["metadata"], (
                f"Metadata changed for {memory.content_hash[:8]}"
            )

            # Memory type preserved
            assert memory.memory_type == before_data["memory_type"], (
                f"Memory type changed for {memory.content_hash[:8]}"
            )
    ...
    async def test_checkpoint_resume_mid_batch(self, temp_storage_path, checkpoint_path, monkeypatch):
        ...
        final_memories = await new_storage.get_all_memories()
        assert len(final_memories) == 200, (
            f"Final count mismatch: expected 200, got {len(final_memories)}"
        )

        print(f"✓ Checkpoint resume validated:")
    ...
    async def test_collection_swap_atomic(self, temp_storage_path, monkeypatch):
        ...
        assert result.get('new_collection') == "memories", (
            "New collection should be renamed to 'memories'"
        )

        old_collection_name = result.get('old_collection')
        assert old_collection_name is not None, "Backup collection name missing"
        assert old_collection_name.startswith("memories_backup_"), (
            f"Backup collection name incorrect: {old_collection_name}"
        )

        print(f"✓ Collection swap validated:")
    ...
    async def test_rollback_restore_backup(self, temp_storage_path, monkeypatch):
        ...
        original_embeddings = {m.content_hash: m.embedding for m in original_memories}
    ...
    async def test_hardware_acceleration(self, temp_storage_path, monkeypatch):
        ...
        print(f"✓ Hardware acceleration test:")
