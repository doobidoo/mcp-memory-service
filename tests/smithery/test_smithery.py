...
def test_smithery_config():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(
            f"Environment: {json.dumps({k: v for k, v in test_env.items() if k.startswith('MCP_') or k in ['PYTHONUNBUFFERED', 'PYTORCH_ENABLE_MPS_FALLBACK']}, indent=2)}"
        )

        try:
            result = subprocess.run(cmd, env=test_env, capture_output=True, text=True, timeout=30)
