"""Storage-bounded stale pagination regressions."""
...
@pytest.mark.asyncio
async def test_hybrid_forwards_all_stale_pagination_arguments():
    """Hybrid delegates the complete stale cursor contract to primary storage."""
    storage = HybridMemoryStorage.__new__(HybridMemoryStorage)
    sentinel = [object()]
    storage.primary = type(
        "Primary",
        (),
        {"get_all_memories": AsyncMock(return_value=sentinel)},
    )()

    result = await storage.get_all_memories(
        limit=5,
        offset=10,
        memory_type="note",
        tags=["tag"],
    ...
    storage.primary.get_all_memories.assert_awaited_once_with(
        limit=5,
        offset=10,
        memory_type="note",
        tags=["tag"],
