...
async def test_coverage_with_real_retrieve_existing_and_novel():
    ...
    # Use real sessions directory but mock harvest to control candidates
    ...
    # If there are gaps, not safe to delete
    if report["missing_insights"] or report["low_quality_matches"]:
        assert report["safe_to_delete"] is False


async def test_empty_session_coverage_is_complete():
    """Real test with empty candidates → should get full coverage and safe to delete."""
    # Dynamic imports to handle missing dependencies gracefully
    try:
        from mcp_memory_service.storage.sqlite_vec import SqliteVecMemoryStorage
        from mcp_memory_service.services.memory_service import MemoryService
    ...
    # Empty session should be completely covered and safe to delete (RFC R11/R12)
