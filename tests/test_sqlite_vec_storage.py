...
class TestSqliteVecStorage:
    ...
    async def test_store_memory(self, storage, sample_memory):
        ...
        cursor = storage.conn.execute("SELECT content_hash FROM memories WHERE content_hash = ?", (sample_memory.content_hash,))
    ...
    async def test_delete_by_tag(self, storage):
        ...
        memory1 = Memory(content="Memory 1", content_hash=generate_content_hash("Memory 1"), tags=["tag1", "shared"])
        memory2 = Memory(content="Memory 2", content_hash=generate_content_hash("Memory 2"), tags=["tag2", "shared"])
        memory3 = Memory(content="Memory 3", content_hash=generate_content_hash("Memory 3"), tags=["tag3"])
    ...
    async def test_update_memory_metadata(self, storage, sample_memory):
        ...
        success, message = await storage.update_memory_metadata(content_hash=sample_memory.content_hash, updates=updates)
    ...
    async def test_update_nonexistent_memory(self, storage):
        ...
        nonexistent_hash = "nonexistent123456789"
        success, message = await storage.update_memory_metadata(content_hash=nonexistent_hash, updates={"tags": ["test"]})
    ...
    async def test_update_memory_with_invalid_tags(self, storage, sample_memory):
        ...
        success, message = await storage.update_memory_metadata(
            content_hash=sample_memory.content_hash, updates={"tags": "not_a_list"}
        )
    ...
    async def test_update_memory_with_invalid_metadata(self, storage, sample_memory):
        ...
        success, message = await storage.update_memory_metadata(
            content_hash=sample_memory.content_hash, updates={"metadata": "not_a_dict"}
        )
    ...
    async def test_update_memory_preserve_timestamps(self, storage, sample_memory):
        ...
        success, message = await storage.update_memory_metadata(
            content_hash=sample_memory.content_hash, updates={"tags": ["updated"]}, preserve_timestamps=True
        )
    ...
    async def test_update_memory_reset_timestamps(self, storage, sample_memory):
        ...
        success, message = await storage.update_memory_metadata(
            content_hash=sample_memory.content_hash, updates={"tags": ["updated"]}, preserve_timestamps=False
        )
    ...
    async def test_multiple_memories_retrieval(self, storage):
        ...
        for i in range(5):
            ...
            memory = Memory(content=content, content_hash=generate_content_hash(content), tags=[f"tag{i}"], memory_type="note")
    ...
    async def test_concurrent_operations(self, storage):
        ...
        for i in range(10):
            ...
            memory = Memory(content=content, content_hash=generate_content_hash(content), tags=[f"concurrent{i}"])
            memories.append(memory)
        ...
        for memory in memories:
            cursor = storage.conn.execute("SELECT content_hash FROM memories WHERE content_hash = ?", (memory.content_hash,))
            assert cursor.fetchone() is not None
...
if __name__ == "__main__":
    ...
    async def run_basic_tests():
        ...
        try:
            ...
            memory = Memory(content=content, content_hash=generate_content_hash(content), tags=["test", "basic"])
