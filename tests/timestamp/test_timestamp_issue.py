...
async def test_timestamp_issue():
    ...
    test_queries = ["yesterday", "last week", "2 days ago", "last month", "this morning", "yesterday afternoon"]
    ...
    memory = Memory(content="Test memory", content_hash=generate_content_hash("Test memory"), tags=["test"])
    ...
    print(
        f"  Would be included: {int(test_memory_timestamp) >= int(yesterday_start) and int(test_memory_timestamp) <= int(yesterday_end)}"
    )
    ...
    if int(edge_timestamp) == int(yesterday_start) and edge_timestamp > yesterday_start:
        print("  WARNING: This memory would be missed with int comparison!")
