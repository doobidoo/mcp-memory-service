...
class TestExactTagMatching:
    ...
    async def test_exact_tag_match_no_false_positives(self, temp_db_path):
        ...
        for result in results:
            assert "python" in result.memory.tags, \
                f"Result should have 'python' tag, got {result.memory.tags}"
            # Verify it's NOT one of the false positives
            assert "python3" not in result.memory.tags, \
                f"Should not match 'python3', got {result.memory.tags}"
            assert "cpython" not in result.memory.tags, \
                f"Should not match 'cpython', got {result.memory.tags}"
            assert "jython" not in result.memory.tags, \
                f"Should not match 'jython', got {result.memory.tags}"
    ...
    async def test_multi_tag_exact_matching(self, temp_db_path):
        ...
        assert len(results) == 3, \
            f"Expected 3 results with 'python' OR 'java' tags, got {len(results)}"
        ...
        for result in results:
            ...
            assert has_python or has_java, \
                f"Result should have 'python' or 'java' tag, got {result.memory.tags}"
            assert not has_python3, \
                f"Should not match 'python3', got {result.memory.tags}"
            assert not has_javascript, \
                f"Should not match 'javascript', got {result.memory.tags}"
    ...
    async def test_edge_cases_single_tag(self, temp_db_path):
        ...
        assert len(results) == 4, \
            f"Expected 4 results with 'python' tag, got {len(results)}"
