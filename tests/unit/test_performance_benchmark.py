...
class TestPerformanceBenchmark:
    ...
    async def test_tag_filtering_performance_10k_memories(self, temp_db_path):
        ...
        for result in results:
            assert "python" in result.memory.tags, \
                f"All results should have 'python' tag, got {result.memory.tags}"
        ...
        assert query_duration < 0.5, \
            f"Multi-tag query took {query_duration*1000:.2f}ms (should be <500ms)"
        ...
        assert query_duration < 0.5, \
            f"Combined filter query took {query_duration*1000:.2f}ms (should be <500ms)"
    ...
    async def test_search_by_tag_performance_10k(self, temp_db_path):
        ...
        assert query_duration < 0.2, \
            f"search_by_tag took {query_duration*1000:.2f}ms (should be <200ms)"

        # Verify correctness
        assert len(results) == 2000, \
            f"Should return 2000 results (10K / 5 tags), got {len(results)}"
    ...
    async def test_count_performance_at_scale(self, temp_db_path):
        ...
        assert query_duration < 0.1, \
            f"count_all_memories took {query_duration*1000:.2f}ms (should be <100ms)"
    ...
    async def test_performance_comparison_old_vs_new(self, temp_db_path):
        ...
        start = time.time()
        results_new = await storage.retrieve(
            query="tutorial",
            n_results=10,
            tags=["python"]
        )
